...
public class ContentIdeaJainImpl extends MBaseModelImpl implements IReaderBookModel {
    ...
    public Observable<List<SearchBookBean>> searchBook(String content, int page) {
        return getRetrofitObject(TAG).create(IIdeaJianAPI.class).searchBook(content).flatMap((Function<String, ObservableSource<List<SearchBookBean>>>) s -> parseSearchBook(s));
    }
...
//                            item.setState();
    ...
    public Observable<CollectionBookBean> getBookInfo(CollectionBookBean collBookBean) {
        return getRetrofitObject(TAG).create(IIdeaJianAPI.class).getBookInfo(collBookBean.get_id().replace(TAG, "")).flatMap(new Function<String, ObservableSource<CollectionBookBean>>() {
            @Override
            public ObservableSource<CollectionBookBean> apply(String s) throws Exception {
                return parseBookInfo(s, collBookBean);
            }
        });

    }

    private Observable<CollectionBookBean> parseBookInfo(final String s, final CollectionBookBean collBookBean) {
        return Observable.create(e -> {
    ...
    public Single<ChapterInfoBean> getChapterInfo(String url) {
        ...
        return getRetrofitObject(TAG).create(IIdeaJianAPI.class).getChapterInfo(url).flatMap(new Function<String, SingleSource<? extends ChapterInfoBean>>() {
            ...
            public SingleSource<? extends ChapterInfoBean> apply(String s) throws Exception {
                return Single.create(emitter -> emitter.onSuccess(analysisChapterInfo(s, url)));
            }
        });
    }
